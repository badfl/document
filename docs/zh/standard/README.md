...
```
构造函数：通过`new`运算符创建新对象的函数。应当以驼峰格式命名并且**首字符大写**。构造函数名称应当以非动词开头，因为`new`代表着创建一个对象实例的操作。
```js
// 好的写法
function MyObject() {
    // 代码
...
### 括号间距
当使用括号时，紧接左括号（`(`）之后和紧接右括号（`)`）之前不应该有空格。
```js
// 好的写法
var found = (values[i] === item);
...
对象直接量应当使用如下格式：
* 起始左花括号（`{`）应当同表达式保持同一行。
* 每个属性的名值对应应当保持一个缩进，第一个属性应当在左花括号（`{`）后另起一行。
...
```
可以使用二个方括号（`[]`）将数组初始元素括起来，来替代使用`Array`构造函数的方式来创建数组。
```js
// 好的做法
...
### 变量声明
所有的变量在使用前都应当事先定义，变量定义应当放在函数开头，每个`var`关键字单独声
明一个变量，每行一个变量。不要声明无用的变量。
变量定义时应当初始化。**初始化的变量应当在未初
始化变量之前**。
```js
// 好的写法
var count = 10;
